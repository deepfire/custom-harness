  ...
  (let ((*print-right-margin* 80)
    ...
    (pprint-logical-block (stream tests :prefix (format nil "running tests from suite ~S:" (name suite)))
      ...
      (iter (for test-spec = (pprint-pop))
            (while test-spec)
            (destructuring-bind (test-name &key expected-failure unstable-failure required-features) (ensure-cons test-spec)
              (if-let ((missing (set-difference required-features *test-critical-features* :test #'eq)))
                (format-test stream test-name (format nil "SKIPPED, missing features:~{ ~A~}" missing))
